...
namespace HockeyApp.AppLoader.PlatformStrategies
{
	...
	public class AppInfoMatcher
	{
		...
		public async Task<List<AppInfo>> GetMatchingApps(CommandLineArgs args)
		{
			...
			{
				Console.WriteLine("No matching apps - using platform-fallback!");
		...
		{
			// 2017-06-26 SaS: It looks like HockeyApp wants to get UWP-APPs under the Windows-Label
			List<AppInfo> apps = this._envelope.Apps.Where(p => p.Platform == AppInfoPlatforms.WindowsPhone || p.Platform == AppInfoPlatforms.Windows).ToList();

			//ZipArchive zip = null;
			IEnumerable<ZipArchiveEntry> zipEntries;
			if ( _args.Package.ToUpper().EndsWith("APPXUPLOAD") )
			{
				var outerZip = new ZipArchive(File.OpenRead(_args.Package), ZipArchiveMode.Read, false);
				var zip = GetInnerPackageFromBundle(new ZipArchive(outerZip.Entries.Single(entry => entry.Name.EndsWith("appxbundle", StringComparison.InvariantCultureIgnoreCase)).Open()).Entries);
				zipEntries = zip.Entries;
			}
			else
			if ( _args.Package.ToUpper().EndsWith("APPXBUNDLE") )
			{
				var zip = new ZipArchive(File.OpenRead(_args.Package), ZipArchiveMode.Read, false);
				zip = GetInnerPackageFromBundle(zip.Entries);
				zipEntries = zip.Entries;
			}
			else
			{
		...
		private ZipArchive GetInnerPackageFromBundle(IEnumerable<ZipArchiveEntry> zipEntries)
		{
		...
		private void MatchIOS(CommandLineArgs args)
		{
			List<AppInfo> apps = this._envelope.Apps.Where(p => p.Platform == AppInfoPlatforms.iOS).ToList();
			var idMatch = apps.First(app => app.PublicID == args.Id);

			var zip = new ZipArchive(File.OpenRead(this._args.Package), ZipArchiveMode.Read, false);
			foreach ( ZipArchiveEntry zipEntry in zip.Entries )
			{
				if ( zipEntry.Name.Equals("Info.plist") && Path.GetDirectoryName(zipEntry.FullName).EndsWith(".app", StringComparison.InvariantCultureIgnoreCase) )
				{
					using ( var zipEntryStream = zipEntry.Open() )
					using ( var seekableStream = new MemoryStream() )
					using ( var decompressedPList = new MemoryStream() )
					{
						zipEntryStream.CopyTo(seekableStream);
						seekableStream.Position = 0;

						var plist = PList.Load(seekableStream);
						PList.Save(plist, decompressedPList, PListFormat.Xml);
						decompressedPList.Position = 0;

						var rootXml = XElement.Load(XmlReader.Create(decompressedPList, new XmlReaderSettings { DtdProcessing = DtdProcessing.Parse }));
						var elements = rootXml.Descendants("dict").Elements();
						var keyElement = elements.Single(key => key.Value == "CFBundleVersion");
						var valueElement = keyElement.NodesAfterSelf().OfType<XElement>().First();
						idMatch.Version = valueElement.Value;
					}
			...
			this._matchingApps.Add(idMatch);
		}
		...
		private Dictionary<string, string> GetManifestInfo()
		{
			...
			{
				throw new Exception(string.Format("AAPT-Tool not found. Please check configuration in '{0}'! (searched AAPT in '{1}')", App.UserFolder, aaptFile));
			}
